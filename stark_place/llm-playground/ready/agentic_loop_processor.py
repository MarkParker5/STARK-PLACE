...
async def respond(
    ...
    """
    response = Response(text=text, voice=text, needs_user_input=needs_user_input)
    ...
    for cmd in commands:
        ...
        def _make_tool_fn(bound_cmd: Command, bound_params: dict) -> Callable:
            ...
            _tool_fn.__annotations__ = {
                "ctx": RunContext[_Deps],
                **{name: (p.annotation if p.annotation is not inspect.Parameter.empty else str) for name, p in bound_params.items()},
                "return": str,
            }
            return _tool_fn
    ...
    async with _agent.iter(
        ...
        if final:
            ...
            if final.output and final.output.strip():
                await context.respond(Response(text=final.output, voice=final.output))
