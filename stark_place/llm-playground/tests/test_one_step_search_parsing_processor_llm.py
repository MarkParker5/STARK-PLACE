...
async def test_parameters_guaranteed_with_none_fallback(make_context):
    ...
    async with make_context(processors=[processor], object_types=[Lamp, BrightnessLevel]) as (manager, context, collector):
        layer = CommandsContextLayer(commands=[cmd], parameters={})
        context.context_queue = [layer]
...
async def test_multi_command_both_returned(make_context):
    ...
    async with make_context(processors=[processor], object_types=[Lamp, BrightnessLevel]) as (manager, context, collector):
        ...
        context.context_queue = [layer]
        results = await processor.process_context_layer("turn on bedroom lamp and make the brightness 50%", context, layer, [])
...
async def test_natural_phrasing_matched(make_context):
    ...
    async with make_context(processors=[processor], object_types=[Lamp, BrightnessLevel]) as (manager, context, collector):
        layer = CommandsContextLayer(commands=[cmd], parameters={})
        context.context_queue = [layer]
        results = await processor.process_context_layer("could you maybe lower the lights in here a bit", context, layer, [])
