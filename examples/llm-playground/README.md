...
Even a small embeddings model performs expectedly good in finding the closest command name to the request (which never was the main problem, but still). However, embeddings are unable to neither parse parameters, nor perform NER (the main problem).

Details: Trigger words often match sloser to the parameter type then the parameter. For example, in "play bohemian rapsody song" words "play" and "song" match closer to "Song" class then "bohemian rapsody". Embeddings-powered intent suggestions might work, but NER is defenitely will stay in drafts.
...
The extraction of parameters is the biggest challenge, which is expected. However, it is the primary reason for using a LLM at all.
...
- ask to use full string
- prompt better reasoning with specific structure; to self-reflect and self-correct at reasoning stage; find all params and bounds before filling json
- better tests: outside regex but specific and understandable; a few in one; delayed actions, automations, when-if-then once, disable next run; creative, not specific - decision making
- refactor prompts
