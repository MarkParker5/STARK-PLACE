    ...
    async def _search_command(self, string: str, context_layer: CommandsContextLayer) -> list[SearchResult]:
        try:
            response = await search_agent.run(
                string, deps=LLMCommandSearchDeps(current_context_commands=[CommandInfo.from_command(cmd) for cmd in context_layer.commands])
            )
        ...
        for match in matches:
            if match.command_name not in cmd_name_to_cmd:
                dev_raise(f"LLM returned unknown command name: {match.command_name!r}")
                continue
